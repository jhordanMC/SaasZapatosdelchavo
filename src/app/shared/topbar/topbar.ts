...
export class TopbarComponent implements OnInit, OnDestroy {
  ...
  /**
   * trackBy para los *ngFor del dropdown de búsqueda. SIN esto, Angular
   * recreaba los <button> en CADA (mouseenter) — resultadosNavegacion es
   * un getter que arma objetos {} nuevos cada vez que corre detección de
   * cambios (y el mouseenter de cada ítem dispara una corrida), así que
   * sin trackBy el *ngFor veía "objetos distintos" y destruía/recreaba
   * los botones en pleno hover. Si esa recreación caía justo entre el
   * mousedown y el mouseup del click del usuario, el navegador no
   * llegaba a disparar el evento click (el elemento original ya no
   * estaba en el DOM) — por eso el click en una recomendación "no hacía
   * nada". Con trackBy por label+ruta, Angular reutiliza el mismo
   * elemento mientras represente el mismo resultado, y el click deja de
   * perderse.
   */
  trackByResultado(_index: number, r: ResultadoBusqueda): string {
    return `${r.tipo}|${r.label}|${r.ruta.join('/')}`;
  }
